...
for d in (LANDSAT_DIR, SENTINEL_DIR, GLIMS_DIR, DEM_DIR, OUTPUTS_DIR,
          IG_OUT_DIR, PAPER_OUT_DIR, GLOBAL_OUT_DIR):
...
def get_glacier(key):
    ...
    # Fuzzy match on name
    for k, v in GLACIER_REGISTRY.items():
        if key in v["name"].lower():
            return v
    raise KeyError(
        f"Glacier '{key}' not found in registry. "
        f"Available: {', '.join(GLACIER_REGISTRY.keys())}"
    )
...
def make_custom_glacier(name, lat, lon, bbox_pad_deg=0.2, hemisphere=None, season=None):
    ...
    return {
        ...
        "region": "Custom",
        "lat": lat, "lon": lon,
        "bbox": (lon - bbox_pad_deg, lat - bbox_pad_deg,
                 lon + bbox_pad_deg, lat + bbox_pad_deg),
