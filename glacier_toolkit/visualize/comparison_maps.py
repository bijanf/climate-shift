...
from ..config import (
    C_BG, C_TEXT, C_SUB, C_LIGHT, C_ACC, C_ICE,
...
def make_comparison_slide(
    ...
    if early_rgb is not None:
        ax1.imshow(np.clip(early_rgb * 1.5, 0, 1), extent=extent,
                   aspect="auto", interpolation="bilinear")
    _overlay_mask_outline(ax1, early_mask, color=C_ICE, linewidth=1.5,
                          extent=extent)
    ax1.text(0.02, 0.95, str(year_early), transform=ax1.transAxes,
             fontsize=22, fontweight="bold", color=C_TEXT, va="top",
             family=FONT_FAMILY,
             bbox=dict(boxstyle="round,pad=0.3", facecolor=C_BG, alpha=0.7,
                       edgecolor="none"))
    ax1.text(0.02, 0.05, f"{area_early_km2:.1f} km²",
             transform=ax1.transAxes, fontsize=14, color=C_ICE, va="bottom",
             family=FONT_FAMILY)
    ax1.axis("off")
    ...
    if late_rgb is not None:
        ax2.imshow(np.clip(late_rgb * 1.5, 0, 1), extent=extent,
                   aspect="auto", interpolation="bilinear")
    # Show the OLD boundary as dashed (ghost) and new as solid
    _overlay_mask_outline(ax2, early_mask, color=C_ICE, linewidth=1.0,
                          linestyle="--", extent=extent)
    _overlay_mask_outline(ax2, late_mask, color="#FFFFFF", linewidth=1.5,
                          extent=extent)
    ax2.text(0.02, 0.95, str(year_late), transform=ax2.transAxes,
             fontsize=22, fontweight="bold", color=C_TEXT, va="top",
             family=FONT_FAMILY,
             bbox=dict(boxstyle="round,pad=0.3", facecolor=C_BG, alpha=0.7,
                       edgecolor="none"))
    ax2.text(0.02, 0.05, f"{area_late_km2:.1f} km²",
             transform=ax2.transAxes, fontsize=14, color=C_ACC, va="bottom",
             family=FONT_FAMILY)
    ax2.axis("off")

    # ── Divider line between panels ──
    from matplotlib.lines import Line2D
    line = Line2D([0.05, 0.95], [0.455, 0.455], color=C_LIGHT, linewidth=0.8,
...
def _overlay_mask_outline(ax, mask, color, linewidth, linestyle="-",
                          extent=None):
