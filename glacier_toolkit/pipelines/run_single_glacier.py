...
def main():
    parser = argparse.ArgumentParser(description="Analyze glacier retreat from satellite imagery")
    parser.add_argument(
        "--name", type=str, help="Glacier registry key (e.g. 'aletsch', 'columbia')"
    )
    ...
    parser.add_argument(
        "--skip-download", action="store_true", help="Skip satellite data download (use cached)"
    )
    ...
    from glacier_toolkit.config import OUTPUTS_DIR, get_glacier, make_custom_glacier
    ...
    print("  Saved: area_timeseries.csv")
    ...
    results = {
        ...
        "acceleration": {
            k: float(v) if isinstance(v, (np.floating, float)) else v for k, v in accel.items()
        },
    }
    ...
    from glacier_toolkit.analyze.ndsi import classify_glacier, load_ndsi_geotiff
