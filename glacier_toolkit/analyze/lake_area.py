...
def build_lake_timeseries(annual_lake_data):
    ...
    for year in sorted(annual_lake_data.keys()):
        ...
        total_area = sum(lk["area_km2"] for lk in lakes)
        largest = max((lk["area_km2"] for lk in lakes), default=0)
        records.append(
            {
                "year": year,
...
def compute_lake_growth_rate(lake_ts_df, column="total_lake_area_km2"):
    ...
    if len(years) < 3 or np.all(areas == 0):
        return {"linear_rate_km2_per_year": 0, "linear_r_squared": 0, "doubling_time_years": np.inf}
...
def correlate_with_glacier_retreat(glacier_ts_df, lake_ts_df):
    ...
    pr, pp = stats.pearsonr(merged["area_km2"], merged["total_lake_area_km2"])
    sr, sp = stats.spearmanr(merged["area_km2"], merged["total_lake_area_km2"])
